...
# Checks user enters either an integer between 1 and 200 or a break code.
...
def factor_finder(needs_factoring):
    ...
    if len(all_factors) == 1:
        comment = "1 is UNITY (has only one factor, itself)."

    # Finds out if the number is a prime number.
    elif len(all_factors) == 2:
        comment = f"{to_factor} a prime number"
...
while True:
    ...
    to_factor = integer_checker("\nNumber you want factorized: ")
    # Exit code.
    if to_factor == "xxx":
        break
